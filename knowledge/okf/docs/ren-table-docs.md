...
Source path: `docs/components/ren-table.html`
    ...
    <main class="dx-content">
      ...
      <section class="dx-section" id="overview">
        ...
        <ul>
          ...
          <li><strong>Server-paged datasets bigger than memory.</strong> The built-in pager is client-side; for server pagination, listen to <code>ren-page-change</code> and fetch yourself instead of relying on <code>data-page-size</code>.</li>
        </ul>
...
&lt;/div&gt;</code></pre>
          ...
          <div class="dx-vrow">
            ...
            <div class="dx-vrow-items">
              <p><em>Not implemented in v0.9.</em> The built-in body renders all visible rows at once. For datasets &gt; ~5 000 rows, switch to server-side pagination (listen to <code>ren-page-change</code> and fetch the next page) instead of trying to render everything client-side. A windowed virtualizer is on the v1.0 roadmap.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section class="dx-section" id="api">
        ...
        <table class="dx-api">
          ...
          <tbody>
            <tr><td><code class="dx-api-name">ren-sort-change</code></td><td><code>{ column, direction }</code>. Fires after sort applies. Bubbles.</td></tr>
            <tr><td><code class="dx-api-name">ren-row-select</code></td><td><code>{ rowId, selected, allSelectedIds: string[] }</code>. Bubbles.</td></tr>
            <tr><td><code class="dx-api-name">ren-page-change</code></td><td><code>{ page, pageSize }</code>. Useful for server-side pagination — listen and fetch the page yourself.</td></tr>
            <tr><td><code class="dx-api-name">ren-filter-change</code></td><td><code>{ query, matchedRows }</code>. Fires after the toolbar search filter applies.</td></tr>
          </tbody>
        </table>
      </section>
...
table.addEventListener('ren-sort-change', (e) =&gt; {
  history.replaceState(null, '', `?sort=${e.detail.column}.${e.detail.direction}`);
});</code></pre>

        <h3>Server-side sorting</h3>
        <p>Don't pre-sort the rows. Listen for <code>ren-sort-change</code> and fetch.</p>
        <pre class="dx-pre" tabindex="0"><code>table.addEventListener('ren-sort-change', async (e) =&gt; {
  ...
  delete table.dataset.loading;
        ...
        <h3>Row selection</h3>
        <p>Real checkboxes with <code>aria-label</code>. Listen for <code>ren-row-select</code> to enable bulk actions when at least one row is selected.</p>
...
table.addEventListener('ren-row-select', (e) =&gt; {
  bulkBar.hidden = e.detail.allSelectedIds.length === 0;
});</code></pre>

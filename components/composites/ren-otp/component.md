...
requiredMarkup:
  ...
  - "The host element carries .ren-otp; the component adds it on connect, so <ren-otp> alone is enough."
  - "Name the group with aria-label / aria-labelledby on the host; the component then exposes it as role=\"group\" so the name reaches assistive tech."
...
accessibility:
  required:
    ...
    - "An aria-label on the host is only announced because the component sets role=\"group\" alongside it; do not override that role with a decorative one."

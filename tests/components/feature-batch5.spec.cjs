...
const CHAIN = [
  ...
  ['feature-architecture-layers.html', 'index.html'],
];
...
test.describe('Feature 25–30 Ren10 blocks', () => {
  ...
  test('catalog exposes Feature 1–30 in order and continues from Feature 24', async ({ page }) => {
    ...
    await expect(cards.locator('.bb-card-eyebrow')).toHaveText(Array.from({ length: 30 }, (_, index) => `Feature ${index + 1}`));

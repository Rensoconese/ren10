...
const CHAIN = [
  ...
  ['feature-media-top-columns.html', 'index.html'],
];
...
test.describe('Feature 1–6 Ren10 blocks', () => {
  ...
  test('catalog exposes Feature 1–6 in order and continues from CTA 60', async ({ page }) => {
    ...
    await expect(cards.locator('.bb-card-eyebrow')).toHaveText(BLOCKS.map(({ number }) => `Feature ${number}`));

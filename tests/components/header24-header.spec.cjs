...
const BLOCK = '/templates/blocks/hero-centered-email-capture.html';
const ROOT = '[data-rh24-root]';

test.describe('Relume Header 24 translated to Ren10', () => {
  let server;
  ...
  async function gotoBlock(page, width = 1280, height = 900) {
    ...
    const response = await page.goto(`${server.origin}${BLOCK}?ren10_test=header24`);
    expect(response?.status(), 'Header24 block must exist').toBe(200);
    await expect(page.locator(ROOT)).toBeVisible();
  }

  test('owns the exact centered email anatomy without extras or pristine error', async ({ page }) => {
    await gotoBlock(page);
    const root = page.locator(ROOT);
    await expect(root.locator('section.rh24-hero, .rh24-content.ren-center.ren-stack, h1.rh24-heading, p.rh24-description, form.rh24-form, ren-field, input[type="email"], button[type="submit"], p.rh24-legal, a.rh24-terms-link, p.rh24-status')).toHaveCount(11);
    ...
    await expect(root.locator('img, picture, figure, video, iframe, dialog, nav, header, svg, [class*="logo"], [class*="brand"], [class*="overlay"], [class*="scrim"]')).toHaveCount(0);
    await expect(root).toHaveAttribute('aria-labelledby', 'rh24-heading');
  });
  ...
  for (const width of [320, 390, 639, 640, 767, 1024, 1280]) {
    test(`stays centered, content-height, and overflow-free at ${width}px`, async ({ page }) => {
      ...
      const state = await page.locator(ROOT).evaluate((root) => {
        const section = root.querySelector('.rh24-hero');
        const content = root.querySelector('.rh24-content');
  ...
  test('stacks through 639px and switches to a field-submit row at 640px', async ({ page }) => {
    ...
    let boxes = await page.locator(`${ROOT} ren-field, ${ROOT} .rh24-submit`).evaluateAll((nodes) => nodes.map((node) => node.getBoundingClientRect().toJSON()));
    ...
    boxes = await page.locator(`${ROOT} ren-field, ${ROOT} .rh24-submit`).evaluateAll((nodes) => nodes.map((node) => node.getBoundingClientRect().toJSON()));
  ...
  test('shows one invalid state, clears it on valid input, and announces enhanced success', async ({ page }) => {
    await gotoBlock(page);
    const field = page.locator(`${ROOT} ren-field`); const input = page.locator(`${ROOT} .rh24-email`); const error = page.locator(`${ROOT} [data-error]`); const status = page.locator(`${ROOT} .rh24-status`);
    await page.locator(`${ROOT} .rh24-submit`).click();
  ...
  test('has real distinct form and terms destinations', async ({ page, request }) => {
    await gotoBlock(page);
    const form = page.locator(`${ROOT} form`); const terms = page.locator(`${ROOT} .rh24-terms-link`);
  ...
  test('keeps input, submit, and terms touch-safe with visible keyboard focus', async ({ page }) => {
    ...
    const controls = page.locator(`${ROOT} input, ${ROOT} button, ${ROOT} .rh24-terms-link`);
  ...
  test('keeps theme contrast and reduced-motion transitions coherent', async ({ page }) => {
    ...
    const state = await page.locator(`${ROOT} .rh24-submit`).evaluate((node) => { const s=getComputedStyle(node); const p=getComputedStyle(node.closest('.rh24-hero')); return {duration:s.transitionDuration,color:p.color,bg:p.backgroundColor}; });
  ...
  test('keeps the native required form and terms usable without JavaScript', async ({ browser }) => {
    ...
    await expect(page.locator(`${ROOT} form[action]`)).toHaveCount(1); await expect(page.locator(`${ROOT} input[required]`)).toHaveCount(1); await expect(page.locator(`${ROOT} .rh24-terms-link[href]`)).toHaveCount(1);
  ...
  test('passes axe in pristine, invalid, and success states', async ({ page }) => {
    ...
    await page.locator(`${ROOT} .rh24-submit`).click(); await checkA11y(page, ROOT);
    await page.locator(`${ROOT} .rh24-email`).fill('axe@example.com'); await page.locator(`${ROOT} .rh24-submit`).click(); await checkA11y(page, ROOT);
  });

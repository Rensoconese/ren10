...
const CHAIN = [
  ...
  ['feature-media-evidence-grid.html', 'index.html'],
];
...
test.describe('Feature 31–36 Ren10 blocks', () => {
  ...
  test('catalog exposes Feature 1–36 in order and continues from Feature 30', async ({ page }) => {
    ...
    await expect(cards.locator('.bb-card-eyebrow')).toHaveText(Array.from({ length: 36 }, (_, index) => `Feature ${index + 1}`));
